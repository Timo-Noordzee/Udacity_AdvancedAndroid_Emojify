...
class Emojifier {
    ...
    private static final float THRESHOLD_SMILING = 0.15f;
    ...
    private static void whichEmoji(Face face){
        ...
        if(smiling){
            if(leftEyeOpen != rightEyeOpen){
                emoji = (leftEyeOpen) ? Emoji.LeftWink : Emoji.RightWink;
            } else {
                emoji = (leftEyeOpen) ? Emoji.Smiling : Emoji.ClosedEyeSmiling;
            }
        } else {
            if(leftEyeOpen != rightEyeOpen){
                emoji = (leftEyeOpen) ? Emoji.LeftWinkFrowning : Emoji.RightWinkFrowning;
            } else {
                emoji = (leftEyeOpen) ? Emoji.Frowning : Emoji.ClosedEyeFrowning;
            }
        }
